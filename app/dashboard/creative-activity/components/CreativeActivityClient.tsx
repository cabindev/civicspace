// app/dashboard/creative-activity/components/CreativeActivityClient.tsx
'use client'

import { useState, useTransition } from 'react';
import { Table, Button, message, Modal, Space, Card } from 'antd';
...
import { deleteCreativeActivity } from '@/app/lib/actions/creative-activity/delete';
...
interface CreativeActivityClientProps {
  initialActivities: CreativeActivity[];
}

export default function CreativeActivityClient({ initialActivities }: CreativeActivityClientProps) {
  const [activities, setActivities] = useState<CreativeActivity[]>(initialActivities);
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          ...
          const sortedActivities = result.data.sort((a: CreativeActivity, b: CreativeActivity) => 
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          ...
          message.error(result.error || 'ไม่สามารถลบข้อมูลกิจกรรมสร้างสรรค์ได้');
        }
      } catch (error) {
      ...
      <div className="card-body">
        ...
        <div className="card-actions justify-end mt-4">
          <Link href={`/dashboard/creative-activity/${activity.id}`}>
            <button className="btn bg-green-600 btn-sm text-white hover:bg-green-700 border-green-600 hover:border-green-700">
